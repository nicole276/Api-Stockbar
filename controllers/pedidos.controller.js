...
// GET /api/pedidos - Solo pendientes (estado = 0)
...
exports.getPedidoById = async (req, res) => {
  try {
    const pedido = await pool.query(
      `SELECT v.*, c.nombre as nombre_cliente FROM ventas v LEFT JOIN clientes c ON v.id_cliente=c.id_cliente WHERE v.id_venta=$1 AND v.estado=0`, 
      [req.params.id]
    );
...
// POST /api/pedidos - Crear en estado pendiente (0)
exports.createPedido = async (req, res) => {
  try {
    ...
    // Se crea con estado 0 (Pendiente)
    const ventaResult = await pool.query(
      'INSERT INTO ventas (id_cliente, fecha, total, estado) VALUES ($1, NOW(), $2, 0) RETURNING *',
      [id_cliente, total]
    );
...
exports.updatePedido = async (req, res) => {
  try {
    ...
    if (check.rows[0].estado !== 0) return res.status(400).json({ success: false, message: 'Solo se pueden editar pedidos pendientes' });
...
// PATCH /api/pedidos/:id/estado - Completar (0 → 1) y descontar stock
exports.completePedido = async (req, res) => {
  try {
    const { id } = req.params;
    const check = await pool.query('SELECT estado FROM ventas WHERE id_venta = $1', [id]);
    if (check.rows.length === 0) return res.status(404).json({ success: false, message: 'Pedido no encontrado' });
    if (check.rows[0].estado !== 0) return res.status(400).json({ success: false, message: 'El pedido ya fue completado o anulado' });
